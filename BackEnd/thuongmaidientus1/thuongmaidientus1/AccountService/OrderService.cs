...
namespace thuongmaidientus1.AccountService
{
    ...
    public class OrderService : IOrderService
    {
        ...
        public async Task<PayLoad<XulydonhangDTO>> AddDonHangXuLy(IList<XulydonhangDTO> xulydonhangs)
        {
            try
            {
                var listXulydonhang = new List<Xulydonhang>();
                ...
                for(var i = 0; i <  xulydonhangs.Count; i++)
                {
                    ...
                    var checkVanchuyen = _dbcontext.vanchuyens.Where(x => x.id == data.vanchuyen_id).FirstOrDefault();

                    var checkDonhang = _dbcontext.xulydonhangs.Where(x => x.product_id == data.product_id && !x.Deleted).FirstOrDefault();
                    if(checkDonhang != null)
                    {
                        ...
                        _dbcontext.xulydonhangs.Update(checkDonhang);
                    }
                    else
                    {
                        var checkIdProduct = _dbcontext.products.Where(x => x.id == data.product_id && !x.Deleted).FirstOrDefault();
                        ...
                        if (checkIdProduct != null && checkAccount != null)
                        {
                            ...
                            var MapXuLyDonHang = _mapper.Map<Xulydonhang>(data);
                ...
                _dbcontext.xulydonhangs.AddRange(listXulydonhang);
        ...
        public async Task<PayLoad<Order>> AddOrder(string? name, IList<XulydonhangDTO> donhang)
        {
            try {
                ...
                if(await _dbcontext.SaveChangesAsync() > 0)
                {
                    ...
                    {
                        var data = donhang[i];
                        var checkProduct = _dbcontext.products.Where(x => x.id == data.product_id && !x.Deleted).FirstOrDefault();
        ...
        public async Task<PayLoad<object>> FindAllDonHangXuLy(string? name, int page = 1, int pageSize = 20)
        {
            try
            {
                ...
                if (checkAccount != null)
                {
                    var data = _dbcontext.xulydonhangs.Include(r => r.Account).Include(v => v.vanchuyen).Where(x => !x.Deleted).Select(xs => new
                    {
                        id = xs.id,
                    ...
                    else if(checkAccount.role.name == "Shop")
                    {
                        ...
                        if(checkShop != null)
                        {
                            ...
                            //{
                            //    var checkProduct = _dbcontext.products.Include(s => s.Shops).Include(a => a.Accounts).Where(x => x.id == item.product_id && x.Shops.Equals(checkShop)).FirstOrDefault();
                            //    if (checkProduct != null)
                            //    {
                            //        listData.Add(item);
                            //    }
                            ...
                            // Cách 2
                            var checkProdut = _dbcontext.products.Include(s => s.Shops).Include(a => a.Accounts).Where(x => x.Shops.Equals(checkShop) && !x.Deleted).ToList();
                            for(var i = 0; i < checkProdut.Count; i++)
                            {
                                var dataItem = checkProdut[i];
                                var checkChoXacNhan = _dbcontext.xulydonhangs.Include(a => a.Account).Include(v => v.vanchuyen).Where(x => x.product_id == dataItem.id && !x.Deleted).Select(d => new
                                {
                                    id = d.id,
                    ...
                    {
                        //var CheckXuLyDonHangUser = _dbcontext.xulydonhangs.Include(a => a.Account).Where(x => x.Account.Equals(checkAccount)).Select(xs => new
                        //{
                        //    id = xs.id,
                        //    product_id = xs.product_id,
                        //    product_name = xs.product_name,
        ...
        public async Task<PayLoad<XulydonhangDTO>> UpdateDonHangXuLy(int product_id, string? name, string? account_name, Status? status = Status.Pending)
        {
            try
            {
                var checkProduct = _dbcontext.xulydonhangs.Include(a => a.Account).Where(x => x.product_id == product_id && !x.Deleted).FirstOrDefault();
                ...
                if(checkProduct != null && checkAccount != null)
                {
                    ...
                    {
                        var checkXuLyDH = _dbcontext.xulydonhangs.Where(x => x.product_id == product_id && !x.Deleted).FirstOrDefault();
                        if(checkXuLyDH != null)
                        {
                            ...
                            _dbcontext.xulydonhangs.Update(checkXuLyDH);
                        }
                    }
                    if(status != Status.Delete && status != Status.Done)
                    {
                        ...
                        checkProduct.CretorEdit = checkAccount.username + " là người đã sửa bản ghi này vào lúc " + DateTimeOffset.UtcNow;
                        _dbcontext.xulydonhangs.Update(checkProduct);
                    }
