...
namespace thuongmaidientus1.AccountService
{
    public class AccountService : IAccountService
    {
        ...
        public async Task<string> Action(string code)
        {
            ...
            var checkAccount = _dbcontext.accounts.Where(x => x.Equals(checkCode.account)).FirstOrDefault();
        ...
        public async Task<AccountDTO> AddAccount(AccountDTO accountDTO)
        {
            ...
            var existingAccount = _dbcontext.accounts.FirstOrDefault(x => x.username == accountDTO.username && x.email == accountDTO.email);
        ...
        public async Task<string> DeleteAccount(IList<string> id)
        {
            ...
            try
            {
                foreach (var item in id)
                {
                    ...
                    {
                        var checkIdList = _dbcontext.accounts.Where(x => x.id == songuyen).ToList();
                        if(checkIdList != null)
                        {
                        ...
                        //_dbcontext.accounts.RemoveRange(checkIdList);
                    }
                    else if(!checkInt) {
                        var chuoi = item.Trim();
                        //var checkList = _dbcontext.accounts.Where(x => x.username.Contains(chuoi)).ToList();
                        var checkList = _dbcontext.accounts.Where(x => x.username == chuoi).ToList();
                        if(checkList != null)
                        {
                        ...
                        //_dbcontext.accounts.RemoveRange(checkList);
                    }
        ...
        public async Task<AccountDTO> EditAccount(int id, AccountDTO accountDTO)
        {
            try
            {
                ...
                var checkAccountToken = _dbcontext.accounts.Where(x => x.Equals(checkToken.account)).FirstOrDefault();
                char[] kytu = new char[] { ',' };
                ...
                var checkId = _dbcontext.accounts.Where(x => x.id == accountDTO.id).FirstOrDefault();
                var checkAccountName = _dbcontext.accounts.Where(x => x.username != checkId.username && x.username == accountDTO.username).FirstOrDefault();
                
                if(checkAccountName != null) {
                    return await Task.FromResult(new AccountDTO { username = "Null" });
                }
                //var checkName = _dbcontext.accounts.Where(x => x.username == accountDTO.username).FirstOrDefault();
                ...
                var checkAccounts = _dbcontext.accounts.Where(x => x.username == accountDTO.username && x.password == accountDTO.password).FirstOrDefault();
                ...
                if (accountDTO.role_id == null)
                {
                    ...
                    var checkAccount = _dbcontext.accounts.Include(x => x.role).Where(x => x.id == int.Parse(IdAccount)).FirstOrDefault();
                ...
                _dbcontext.accounts.Update(checkId);
                if (await _dbcontext.SaveChangesAsync() > 0)
                {
        ...
        {
            var AccountRole = _dbcontext.accounts.Include(r => r.role).Where(x => !x.Deleted).Select(x => new
            {
                Id = x.id,
                Name = x.username,
                Password = x.password,
        ...
        {
            //string key = "ThisismySecretKey";
            var checkLogins = _dbcontext.accounts.Include(u => u.role).Where(x => (x.username == login.username || x.email == login.username) && x.Action && !x.Deleted).FirstOrDefault();
            //var checkLogins = from a in _dbcontext.accounts
            ...
            //                      roleId = r.id
            //                  };
            var checkLogin = _dbcontext.accounts.Where(x => !x.Deleted).ToList();
            ...
            foreach (var item in checkLogin)
            {
                ...
                if (checkPassword == ByteToStringDto && checkLogins.username == item.username)
                {
                    ...
                    var checkShop = _dbcontext.shops.Include(s => s.account).Where(x => x.account.Equals(item) && !x.Deleted).FirstOrDefault();
        ...
        public async Task<TokenMessage> checkToken(string token)
        {
            ...
            if(tokenreand != null)
            {
                ...
                if(date > dateTokes)
                {
                    ...
                    var checkAccount = _dbcontext.accounts.Include(r => r.role).Where(x => x.id == int.Parse(checkClaim)).FirstOrDefault();
        ...
        public async Task<PayLoad<List<AccountDTO>>> FindOne(IList<string> id)
        {
            ...
            try
            {
                var datas = _dbcontext.accounts.Include(r => r.role).ToList();
                ...
                for(var i = 0; i < id.Count; i++)
                {
                    ...
                    {
                        var dataInt = _dbcontext.accounts.Include(r => r.role).Where(x => x.id.Equals(songuyen)).FirstOrDefault();
                        if(dataInt != null)
                        {
                            list.Add(dataInt);
                        }
