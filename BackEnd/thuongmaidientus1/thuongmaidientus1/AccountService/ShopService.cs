    ...
    public class ShopService : IShopService
    {
        ...
        public async Task<PayLoad<ShopDTO>> AddShop(ShopDTO shop) 
        {
            try
            {
                ...
                var checkNameShop = _dbcontext.shops.Where(x => x.Name == shop.Name).FirstOrDefault();
                ...
                //var checkAccount = _dbcontext.accounts.Where(x => x.Equals(checkNameAccount.account)).FirstOrDefault();
                ...
                var MapShop = _mapper.Map<Shop>(shop);
                MapShop.account= checkAccount;
                MapShop.CretorEdit = checkAccount == null ? "Admin là người đã tạo và sửa Shop này" : "";
                ShopVanchuyen shopVanchuyen = new ShopVanchuyen();
                var sum = 0;
                foreach (var item in shop.vanchuyens)
                {
                    ...
                    {
                        var checkVanchuyen = _dbcontext.vanchuyens.Where(x => x.id == songuyen).FirstOrDefault();
                        if (checkVanchuyen != null)
                        {
                            ...
                            if (int.Parse(intCoverString) > sum - 1)
                            {
                                ...
                                //MapShop.vanchuyen = shop.vanchuyens[0] // "shop.vanchuyens[0]": "[0]" nghĩa là lấy ra phần tử đầu tiên trong mảng;
                            }
                            ...
                            _dbcontext.shopVanchuyens.Add(shopVanchuyen);
                        }
                    }

                    if (!checkChuoi)
                    {
                        var checkVanchuyens = _dbcontext.vanchuyens.Where(x => x.name == item).FirstOrDefault();
        ...
        private Account checkIdOrToken(ShopDTO id)
        {
            //if(id.account_id != 0)
            //{
            //    var checkAccountID = _dbcontext.accounts.Include(r => r.role).Where(x => x.id == id.account_id).FirstOrDefault();
            //    if(checkAccountID.role.name != "Admin")
            //    {
            //        var checkRoleShop = _dbcontext.roles.Where(x => x.name == "Shop").FirstOrDefault();
            //        checkAccountID.role = checkRoleShop;
            //        _dbcontext.accounts.Update(checkAccountID);
            ...
            //    return null;
            //}
            //var checkAccountByToken = _dbcontext.accounts.Include(r => r.role).Where(x => x.Equals(checkTokenAccount.account)).FirstOrDefault();
            ...
            //{
            //    var checkRoleShops = _dbcontext.roles.Where(x => x.name == "Shop").FirstOrDefault();
            //    checkAccountByToken.role = checkRoleShops;
            //    _dbcontext.accounts.Update(checkAccountByToken);
            //    _dbcontext.SaveChanges();
            //}
            ...
            if(checkToken != null)
            {
                var checkAccount = _dbcontext.accounts.Include(r => r.role).Where(x => x.Equals(checkToken.account)).FirstOrDefault();
                if(checkAccount != null)
                {
                    var checkRole = _dbcontext.roles.Where(x => x.name == "Shop").FirstOrDefault();
                    if (checkAccount.role.name == "Admin")
                    {
                        // Kiểm tra id được chuyền lên
                        var checkAccoutShop = _dbcontext.accounts.Include(rl => rl.role).Where(x => x.id == id.account_id).FirstOrDefault();
                        ...
                        else if(checkAccoutShop.role.name != "Admin")
                        {
                            ...
                            _dbcontext.accounts.Update(checkAccoutShop);
                            _dbcontext.SaveChanges();
                    ...
                    else if(checkAccount.role.name != "Admin")
                    {
                        ...
                        _dbcontext.accounts.Update(checkAccount);
                        _dbcontext.SaveChanges();
                        return checkAccount;
                    }
                }
        ...
        public async Task<PayLoad<string>> DeleteShop(IList<string> id)
        {
            ...
            try
            {
                //var checkIdList = _dbcontext.shops.Where(x => id.Contains(string.Format("{0}", x.id))).ToList();
                var checkIdList = _dbcontext.shops.Where(x => id.Contains(x.id.ToString())).ToList();
                ...
                _dbcontext.shops.UpdateRange(listShop);
                if(await _dbcontext.SaveChangesAsync() > 0) {
        ...
        public async Task<PayLoad<ShopDTO>> EditShop(int id, ShopDTO shop, string? username)
        {
            try
            {
                var checkIdShop = _dbcontext.shops.Where(x => x.id == id).FirstOrDefault();
                if(checkIdShop == null)
                {
                    return await Task.FromResult(PayLoad<ShopDTO>.CreatedFail("Shop không tồn tại"));
                }

                var checkNameShop = _dbcontext.shops.Where(x => x.Name != checkIdShop.Name && x.Name == shop.Name).FirstOrDefault();
                if(checkNameShop != null)
                {
                    return await Task.FromResult(PayLoad<ShopDTO>.CreatedFail("Tên Shop đã không tồn tại"));
                }

                var checAccount = _dbcontext.accounts.Where(x => x.username == username).FirstOrDefault();
                if(checAccount != null)
                {
                ...
                var checkShopVanChuyen = _dbcontext.shopVanchuyens.Include(s => s.shop).Where(x => x.shop == checkIdShop).ToList();
                if(checkShopVanChuyen.Count > 0)
                {
                    _dbcontext.shopVanchuyens.RemoveRange(checkShopVanChuyen);
                    if(await _dbcontext.SaveChangesAsync() <= 0)
                    {
                        return await Task.FromResult(PayLoad<ShopDTO>.CreatedFail("Error Add"));
                    }
                ...
                var listVanChuyenShop = new List<ShopVanchuyen>();
                var sum = 0;
                ...
                for(var i = 0; i < shop.vanchuyens.LongCount(); i++)
                {
                    ...
                    {
                        var checkId = _dbcontext.vanchuyens.Where(x => x.id == songuyen).FirstOrDefault();
                        if(checkId != null)
                        {
                            sum += 1;
                            if(sum == 1)
                            ...
                            listVanChuyenShop.Add(new ShopVanchuyen
                            {
                                shop = checkIdShop,
                                Vanchuyen = checkId
                            });
                    ...
                    {
                        var checkName = _dbcontext.vanchuyens.Where(x => x.name == data).FirstOrDefault();
                        if (checkName != null)
                        {
                            listVanChuyenShop.Add(new ShopVanchuyen
                            {
                ...
                _dbcontext.shopVanchuyens.AddRange(listVanChuyenShop);
                _dbcontext.shops.UpdateRange(checkIdShop);
        ...
        public async Task<PayLoad<object>> FindAll(string? name, int page = 1, int pageSize = 20)
        {
            try
            {
                ...
                var data = _dbcontext.shops.Include(a => a.account).Include(v => v.vanchuyen).Where(x => !x.Deleted).Select(x => new
                {
                    name = x.Name,
                    diachi = x.diachi,
                    email = x.email,
        ...
        public async Task<PayLoad<object>> FindOneIdOrName(IList<string> name)
        {
            try
            {
                ...
                for(var i = 0; i < name.Count; i++)
                {
                    ...
                    if(int.TryParse(data, out int songuyen))
                    {
                        var checkId = _dbcontext.shops.Include(svc => svc.ShopVanchuyens)
                    ...
                    if (!checkInt)
                    {
                        var checkName = _dbcontext.shops.Where(x => x.Name.Contains(data)).ToList();
                        if(checkName != null)
                        {
                            list.Add(checkName); 
                        }
