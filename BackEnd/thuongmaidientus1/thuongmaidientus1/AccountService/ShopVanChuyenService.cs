...
namespace thuongmaidientus1.AccountService
{
    public class ShopVanChuyenService : IShopVanChuyenService
    {
        ...
        public async Task<PayLoad<List<ShopVanchuyenDTO>>> FindOneIdShop(int id)
        {
            try
            {
                var checkIdShop = _dbcontext.shopVanchuyens.Include(s => s.shop).Include(v => v.Vanchuyen).Where(x => x.shop.id == id).Select(x => new
                {
                    Id = x.id,
                    ShopId = x.shop.id,
                ...
                if(checkIdShop == null)
                {
                    return await Task.FromResult(PayLoad<List<ShopVanchuyenDTO>>.CreatedFail("Shop chưa tôn tại"));
                }
